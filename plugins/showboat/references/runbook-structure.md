...
<runbook-dir>/
  ...
  references/
    ...
    api.md              — load when hitting API endpoints
    <other topics>.md
```
...
- **Cross-link liberally**: when content touches another sub-doc's topic, link to the exact section (use anchors like `rodney-patterns.md#waiting-for-network-idle`), not to the file top.
...
- ✅ `see [wait strategies](rodney-patterns.md#waiting-for-network-idle)`
